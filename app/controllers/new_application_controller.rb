class NewApplicationController < ApplicationController
  ...
  def new_application_params
    ...
    # When no radio button is selected on the page no application_type key/value is 
    # passed in the form, meaning that the new_application hash is no longer passed 
    # through either. In this case, we need to add it manually to avoid triggering 
    # a 'param is missing or value is empty' exception
  ...
  # Redirects the user based on the presence of and completeness of their associated 
  # organisation. If not present or incomplete, we redirect to the dashboard
  # orchestration route
  #
  # @param [User] user An instance of User
  def redirect_based_on_organisation_presence_and_completeness(user)

    unless user.organisation && helpers.complete_organisation_details?(user.organisation)
