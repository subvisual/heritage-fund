# Controller for the project enquiry 'project participants' page
class PreApplication::ProjectEnquiry::ProjectParticipantsController < ApplicationController
  include PreApplicationContext, ObjectErrorsLogger
  
  # This method updates the project_participants attribute of a pa_project_enquiry,
  # redirecting to : pre_application_project_enquiry_timescales if successful and
  # re-rendering :show method if unsuccessful
  def update

    logger.info 'Updating project_participants for ' \
    ...
    if @pre_application.pa_project_enquiry.valid?
      ...
      redirect_to(:pre_application_project_enquiry_timescales)

    else

      logger.info 'Validation failed when attempting to update project_participants ' \
  ...
  def pa_project_enquiry_params
    ...
    params.require(:pa_project_enquiry).permit(:project_participants)
