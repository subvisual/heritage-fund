...
RSpec.describe Organisation::MissionsController do
  login_user
  ...
  describe 'PUT #update' do
    ...
    it 'should re-render the page if a combination of invalid and ' \
      ...
      put :update, params: {
        organisation_id: subject.current_user.organisations.first.id,
        organisation: {
          mission: %w[invalid_value female_led]
        }
      }
    ...
    it 'should successfully update if multiple valid params are passed' do
      put :update, params: {
        organisation_id: subject.current_user.organisations.first.id,
        organisation: {
          mission: %w[female_led lgbt_plus_led]
        }
      }
      ...
      expect(assigns(:organisation)
              .mission).to eq(%w[female_led lgbt_plus_led])
    end
  end
end
